...
class EciGenerator(object):
    ...
    def __init__(self, cluster_expansion, structures, energies, weights,
        ...
        Args:
            ...
            solver: solver, current options are cvxopt_l1, gs_preserve
        ...
        if solver not in ('cvxopt_l1', 'gs_preserve'):
    ...
    def _calc_cv_score(self, mu, A, f, weights, k=5):
        ...
        for i in range(k):
            ...
            ecis = self._fit(A[ins], f[ins], weights[ins], mu)
    ...
    def _fit(self, A, f, weights, mu):
        ...
        ecis = self._solve_weighted(A_in, f_in, weights, mu)
    ...
    def _solve_weighted(self, A, f, weights, mu):
        A_w = A * weights[:, None] ** 0.5
        f_w = f * weights ** 0.5

        if self.solver == 'cvxopt_l1':
            return l1regls(A_w, f_w, mu)
    ...
    def _solve_gs_preserve(self, A, f, mu):
        ...
        reduce_composition_at_hull = [
            self.structures[i].composition.reduced_composition.element_composition for
        ...
        for i in range(len(corr_in)):
            ...
            if i not in structure_index_at_hull and i not in duplicated_correlation_set:
                ...
                reduced_comp = self.structures[i].composition.reduced_composition.element_composition
                ...
                if reduced_comp in reduce_composition_at_hull:  ## in hull composition
                    ...
                    G_3_new_line = np.concatenate((new_vector, np.zeros(N_corr)))  # G_3_new_line=np.concatenate((new_vector,np.zeros((self.N_corr))),axis=1)
                ...
                if self.structures[i].composition.is_element:
                ...
                vertices_red_composition_list_new_pd = [pd_new.qhull_entries[i].composition.reduced_composition
